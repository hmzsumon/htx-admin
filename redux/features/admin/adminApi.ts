...
export const adminApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // get all icm-users
    getAllIcmUsers: builder.query<any, any>({
      query: () => ({
        url: "/all-icm-users",
        method: "GET",
      }),
      providesTags: ["IcmUsers"],
    }),

    // get icm user by id
    getIcmUserById: builder.query<any, string>({
      query: (id) => `/icm-user/${id}`,
      providesTags: ["IcmUsers"],
    }),

    // read user mutation
    readUser: builder.mutation<any, any>({
      query: (body) => ({
        url: "/read-user",
        method: "POST",
        body,
